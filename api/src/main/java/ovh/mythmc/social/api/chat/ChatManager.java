...
public final class ChatManager {
    ...
    /**
     * The singleton instance of the chat manager.
     */
    ...
    /**
     * Returns the chat history store for the current session.
     *
     * @return the chat history
     */
    public @NotNull ChatHistory getHistory() {
        return history;
    }
    ...
    public <T> void unregisterRenderer(final @NotNull SocialChatRenderer.Registered<T> registeredRenderer) {
        this.renderersMap.values().removeIf(registered -> registered.equals(registeredRenderer));
    }
    ...
    /**
     * Returns the registered renderer for the given class, or {@code null} if none exists.
     *
    ...
    /**
     * Returns the first renderer able to map the given audience object, or {@code null} if none.
     *
    ...
    /**
     * Returns the default channel as configured in settings.
     */
    public @Nullable ChatChannel getDefault() {
     ...
     */
    public @Nullable ChatChannel getCachedOrDefault(@NotNull SocialUser user) {
        return user.mainChannel().or(getDefault());
    }

    /**
     * Returns the {@link GroupChatChannel} whose name matches {@code "G-" + code}, if any.
     *
     * @param code the numeric group code
     * @return an optional containing the group channel
     */
    public @NotNull Optional<GroupChatChannel> groupChannelByCode(final int code) {
    ...
    /**
     * Returns the {@link GroupChatChannel} the given user is currently a member of, if any.
     *
     * @param user the user to look up
     * @return an optional containing the user's group channel
     */
    public @NotNull Optional<GroupChatChannel> groupChannelByUser(final @NotNull SocialUser user) {
        return Social.registries().channels().valuesByType(GroupChatChannel.class).stream()
     ...
     */
    public @NotNull List<? extends ChatChannel> getVisibleChannels(final @NotNull SocialUser user) {
     ...
     * set and for which they have the required permission.
     *
    ...
    /**
     * Returns {@code true} if the user satisfies the channel's permission requirement.
     *
     * @param user    the user to check
    ...
    public boolean hasPermission(final @NotNull SocialUser user, final @NotNull ChatChannel channel) {
        return channel.permission().map(user::checkPermission).orElse(true);
    }
