...
public final class PlayerHistoryMenu implements HistoryBookMenu {
    ...
    public Book book(SocialHistoryMenuContext context) {
        ...
        for (SocialMessageContext message : Social.get().getChatManager().getHistory().getByPlayer(context.target()).reversed()) {
    ...
    public Component header(SocialHistoryMenuContext context) {
        Component unparsedHeader = Component.text("╒═══════════╕", NamedTextColor.DARK_GRAY)
            .appendNewline()
            ...
            .append(Component.empty()
                ...
                .appendSpace()
                .append(Component.text("$(nickname)", NamedTextColor.BLUE))
            )
            .appendNewline()
            .appendNewline();

        return Social.get().getTextProcessor().parse(
            SocialParserContext.builder()
                .socialPlayer(context.target())
                .message(unparsedHeader)
                .build()
        );
    }
